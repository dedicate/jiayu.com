...
<%
if not((request.cookies(eChsys)("KEY")="super"and request.cookies(eChsys)("purview")="99999")  or Request.cookies(eChsys)("KEY")="checkdep") THEN
	Show_Err("对不起，您无权进行管理操作！")
	response.end
else
	if ((Request.cookies(eChsys)("ManageUserName")="") or (Request.cookies(eChsys)("ManagePasswd")="") or (Request.cookies(eChsys)("ManageKEY")="")) then
		...
		IF not(Request.cookies(eChsys)("ManageKEY")="super" or Request.cookies(eChsys)("ManageKEY")="checkdep") THEN
			if IsDebug="1" then
					Show_Err("cookies(KEY)值不是[ManageSuper][ManageCheckdep]中的一个。<br><br><a href='javascript:history.back()'>返回</a>")
			else
				response.redirect "admin_index.asp?action=Manage.asp"
			end if
			response.end
