...
set rs=server.CreateObject("ADODB.RecordSet")
sql1="select * from "& db_EC_LeadMail_Table &" where topic like '%"&keyword&"%'  and HaveReply="& HaveReply &" order by LeadMailID DESC"
                    ...
                    <td height="300" valign="top">
                      ...
                      <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#FFDFBF" id="AutoNumber1" style="border-collapse: collapse">	
                        <tr class="TDtop1" align="center">
                          <td width="7%" height="25"><span class="top1">状态</span></td>
                          <td width="43%"  class="top1">信件标题</td>
                          <td width="12%" class="top1">来信日期</td>
                        </tr>
                        <%
...
·<a  class=middle href='ShowLeadMail.asp?LeadMailID=<%=rs("LeadMailID")%>'><%=htmlencode(DisplayTopic)%>...</a></td>
		...
		%>
		<tr> 
	<td colspan="5" width="100%" align=center height=25 bgcolor="#ffffff">共搜索到 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 页，每页 <%=MyPageSize%> 条
	<%
...
Response.write "<tr><td align=center bgcolor=#ffffff colspan=3 height=100><font color=red>对不起，没有找到你要搜索的信件！</font><a href=E_leadMailList.asp>返回</a></td></tr>"
